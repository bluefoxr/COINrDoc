...
* `ntype = "minmax"` yields a min-max transformation that scales each indicator onto an interval specified by `npara`, e.g. if `npara = c(0,10)` the indicators will scale to [0, 10].
* `ntype = "zscore"` scales the indicator to have a mean and standard deviation specified by `npara`, e.g. if `npara = c(0,1)` the indicator will have mean zero and standard deviation 1.
* `ntype = "scaled"` is a general linear transformation defined by `npara = c(a,b)` which subtracts `a` and divides by `b`.
...
* `ntype = "custom"` allows to pass a custom function to apply to every indicator. For example, `npara = function(x) {x/max(x, na.rm = T)}` would give the "fracmax" normalisation method described above.
