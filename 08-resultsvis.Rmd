...
resultsTable(ASEM) %>% head(10)
```

By default, `resultsTable()` gives a table of the highest level of aggregation (i.e. typically the index), along with the scores, and the ranks. Scores are rounded to two decimal places, and this can be controlled by the `nround` argument. To see all aggregate scores, we can change the `tab_type` argument:

```{r resultsAgg}
resultsTable(ASEM, tab_type = "Aggregates") %>%
  reactable::reactable()
```
