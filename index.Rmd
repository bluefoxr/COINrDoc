...
*NOTE: this documentation is currently being updated. Should be up to date in the next day or two.*
...
```{r InstallCOINrCRAN, eval=FALSE}
install.packages("COINr")
```

An even more up to date version cab also be installed via GitHub. CRAN versions of the package are only updated every 1-2 months, whereas the GitHub version represents the very latest version available. First, install the **devtools** package if you don't already have it, then run:
