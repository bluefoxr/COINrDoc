...
```{r}
# Get raw data set
datalist <- getIn(ASEM, dset = "Raw", inames = c("Flights", "LPI"))
datalist$ind_data_only
```
...
```{r}
# Get raw data set
datalist <- getIn(ASEM, dset = "Raw", inames = "Political", aglev = 1)
datalist$ind_data_only
```
...
```{r}
# Get raw data set
datalist <- getIn(ASEM, dset = "Aggregated", inames = "Conn", aglev = 1)
datalist$ind_data_only
```
...
```{r}
# Get raw data set
datalist <- getIn(ASEM, dset = "Aggregated", inames = "Conn", aglev = 2)
datalist$ind_data_only
```
...
```{r}
# Get raw data set
datalist <- getIn(ASEM, dset = "Aggregated", inames = "Conn")
datalist$ind_data_only
```
...
datalist <- getIn(ASEMIndData, inames = c("LPI", "Goods"))
datalist$ind_data_only
```
